...
for(const [width,height] of [[2536,696],[1689,697]]){
 ...
 assert.deepEqual(await page.evaluate(()=>calls[0]),['connect','{ABC-123}','',1,1]);
 await page.evaluate(()=>signals.virtualDeviceCreated('other',''));assert(await page.locator('#flight-arm').isDisabled());
 ...
 await page.locator('#flight-connect').click();await page.evaluate(()=>signals.virtualDeviceCreated('{abc-123}',''));await page.locator('#flight-arm').click();
